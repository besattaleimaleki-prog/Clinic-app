...
if not st.session_state.logged_in:
    ...
    if login_type == "ورود مدیر / ادمین 🛠️":
        ...
        if st.session_state.df is None:
            st.warning("⚠️ اطلاعات درمانگاه هنوز توسط مدیر آپلود نشده است. اگر مدیر هستید ابتدا وارد حساب مدیریت شده و فایل اکسل را آپلود کنید.")
            # اجازه آپلود موقت داده‌ها یا فایل توسط مدیر در جلسه جاری
            uploaded_temp = st.file_uploader("آپلود فایل اکسل (در صورت عدم بارگذاری توسط مدیر):", type=["xlsx", "xls"])
            if uploaded_temp:
                st.session_state.df = pd.read_excel(uploaded_temp)
                st.success("فایل با موفقیت بارگذاری شد. اکنون پزشکان می‌توانند وارد شوند.")
                st.rerun()
        else:
            ...
            if st.button("ورود به پنل پزشک", type="primary"):
                if doc_password == DOCTOR_DEFAULT_PASSWORD:
                    st.session_state.logged_in = True
                    st.session_state.user_role = "doctor"
                    st.session_state.doctor_name = selected_doc
...
# ------------------ پنل پس از ورود (Logged-in Panel) ------------------
    ...
    if st.session_state.user_role == "admin":
        ...
        uploaded_file = st.file_uploader("بارگذاری / به‌روزرسانی فایل اکسل درمانگاه", type=["xlsx", "xls"])
        if uploaded_file is not None:
            st.session_state.df = pd.read_excel(uploaded_file)
            st.success("فایل اکسل جدید با موفقیت ذخیره شد.")
        ...
        if st.session_state.df is not None:
            ...
            with tab3:
                st.info("برای چاپ یا ذخیره PDF گزارشات کل، از گزینه Print مرورگر (Ctrl+P) استفاده کنید.")

        else:
            st.info("لطفاً فایل اکسل درمانگاه را آپلود کنید تا داده‌ها تحلیل شوند.")
    ...
    # -------------------------------------------------------------
        ...
        # اطلاعات فقط مربوط به همین پزشک
        doc_data = df[df[doctor_col] == current_doc].iloc[0]
        avg_data = df[metrics].mean()
        
        # محاسبه رتبه بدون نمایش اسامی سایرین
        df_ranks = df.copy()
        for col in metrics:
            if "ویزیت" in col or "آزمایشگاه" in col:
                df_ranks[col] = df[col].rank(ascending=False, method='min').astype(int)
